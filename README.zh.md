...
写一个 `Spider` 并运行 —— **无需数据库、无需 Redis，三步搞定**（单节点内存模式）：
...
**控制面**（选举、分布式锁、成员、分区归属）跑在内嵌的 **redb + Raft** 上，无需外部 Redis。**数据面**保留可插拔消息队列（Kafka / Redis / **NATS JetStream** / 内存），任务按 `hash(account)` 路由实现消费亲和。
...
|---|---|---|
| **控制面** | 进程内 | 内嵌 **redb + Raft**（选举 / 锁 / 成员 / 分区归属）—— **无需 Redis** |
| **队列（数据面）** | Tokio mpsc（内存） | 可插拔 MQ：Kafka / NATS JetStream / Redis Streams / 内存 |
...
也可通过配置提供 Redis（`from_toml`）使用 Redis 协调面而非内嵌 Raft；数据面的消息队列独立选择 —— Kafka（`queue-kafka`）、NATS JetStream（`queue-nats`）、Redis Streams 或内存。

## 特性开关

均为可选;默认构建是**单机、无 DB、无 Redis**。用 `mocra = { version = "0.4", features = ["…"] }` 开启。
...
- [`examples/spider_quickstart.rs`](examples/spider_quickstart.rs) — 最小 `Spider`（无 DB / 无 Redis）
