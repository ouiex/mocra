...
impl EventProcessorTrait<Request, (Request, Option<ModuleConfig>)> for ConfigProcessor {
    fn pre_status(&self, input: &Request) -> Option<SystemEvent> {
        ...
        Some(SystemEvent::ConfigLoad(ConfigLoadPrepared(ev)))
    }
    ...
    fn finish_status(
        ...
        Some(SystemEvent::ConfigLoad(ConfigLoadCompleted(ev)))
    }

    fn working_status(&self, input: &Request) -> Option<SystemEvent> {
        let ev: ConfigLoadEvent = (&(input.clone(), None)).into();
        Some(SystemEvent::ConfigLoad(ConfigLoadStarted(ev)))
    }
    ...
    fn error_status(&self, input: &Request, err: &Error) -> Option<SystemEvent> {
        ...
        Some(SystemEvent::ConfigLoad(ConfigLoadFailed(failure)))
    }
    ...
    fn retry_status(&self, input: &Request, retry_policy: &RetryPolicy) -> Option<SystemEvent> {
        ...
        Some(SystemEvent::ConfigLoad(ConfigLoadRetry(retry)))
    }
}
    ...
    for FlattenStreamVecProcessor<T>
{
    ...
    async fn process(
        ...
        let stream = input.flat_map(move |v| {
            ...
            let iter = v.into_iter().map(move |item| {
