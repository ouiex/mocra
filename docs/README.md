...
single-node, in-memory engine with no database and no Redis. The same code scales to a
...
|---|---|
| [Getting Started](getting-started.md) | Install mocra, write your first `Spider`, and run it — no DB, no Redis |
...
| [Configuration](configuration.md) | Full TOML reference (database, Redis, queues, control API) |
...
- [`spider_quickstart.rs`](../examples/spider_quickstart.rs) — the minimal `Spider` (no DB / no Redis).
