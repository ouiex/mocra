...
（Redis Streams / Kafka / NATS JetStream），同一条流水线都原样运行。横向扩展是配置 / 构建器层面
的事，而非重写。
...
Redis Streams / Kafka / NATS JetStream——**同一份代码，零改动**。变化的只是边界背后的队列后端。
...
|---|---|---|
| **控制面** | 进程内 | 内嵌 **redb + Raft**——选主 / 带栅栏的锁 / 成员管理 / 分区归属，**无需 Redis** |
| **数据面（队列）** | Tokio mpsc（内存） | 可插拔 MQ：Kafka / NATS JetStream / Redis Streams / 内存 |
...
- **内嵌分布式控制面。** 启用 `cluster-embedded`，在首节点加 `.cluster(ClusterConfig::bootstrap(...))`，
  ...
  **redb + Raft** 上——**无需外部 Redis**。把任意节点注册到任意已知节点即可组网。

- **Redis 支撑的分布式控制面。** 不用内嵌集群时，通过 `.from_toml(cfg)` 加载带 `[cache.redis]` 段的
  TOML 配置；协调（锁 / 选主）改走 Redis 而非 Raft。

在上述所有分布式场景中，**数据面队列都独立于控制面选择**——内存、Redis Streams、Kafka
...
- [部署指南](deployment.md)——单节点、内嵌集群、Redis 支撑、监控
