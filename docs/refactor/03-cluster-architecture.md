...
| 数据面接口 | `MqBackend`(保留多实现) | Kafka/Redis 已有,加 NATS 等 |
| 集群 RPC | `tonic`(gRPC)或 `axum` | Raft 节点间通信 + join API;axum 已是依赖,可复用 |
| 成员发现(可选) | `chitchat` / `foca` | gossip 故障探测,Raft 仍为投票真相源 |

## 待处理的耦合

`DistributedLockManager`(`utils/redis_lock.rs`)与限流器(`utils/distributed_rate_limit.rs`)目前**直吃 `deadpool_redis::Pool`、未走 trait**:

- **锁** → 改走 `CoordinationBackend`(→ Raft)。
- **限流** → 改为本地按成员数分摊(数据面热路径,不走共识)。
