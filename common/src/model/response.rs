...
impl crate::model::priority::Prioritizable for Response {
...
#[async_trait]
impl crate::interface::storage::Offloadable for Response {
    fn should_offload(&self, threshold: usize) -> bool {
        self.content.len() > threshold && self.storage_path.is_none()
    }

    async fn offload(&mut self, storage: &std::sync::Arc<dyn crate::interface::storage::BlobStorage>) -> errors::Result<()> {
    ...
    async fn reload(&mut self, storage: &std::sync::Arc<dyn crate::interface::storage::BlobStorage>) -> errors::Result<()> {
