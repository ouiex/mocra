use crate::interface::storage::BlobStorage;
        ...
        if let Some(parent) = path.parent() {
             if !parent.exists() {
                 fs::create_dir_all(parent).await.map_err(|e| {
                     errors::Error::from(errors::error::DataStoreError::SaveFailed(Box::new(e)))
                 })?;
             }
        }
        Ok(())
...
impl BlobStorage for FileBlobStorage {
    ...
    async fn get(&self, key: &str) -> Result<Vec<u8>> {
        ...
        let data = fs::read(&file_path).await.map_err(|e| {
            errors::Error::from(errors::error::DataStoreError::InvalidData(Box::new(e)))
        })?;
        Ok(data)
    }
}
