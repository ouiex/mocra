...
impl NodeRegistry {
    ...
    pub async fn heartbeat(&self, ip: &str, hostname: &str, version: &str) -> Result<(), errors::Error> {
        ...
        info.send_with_ttl(&self.node_id, &self.cache, self.ttl).await.map_err(errors::Error::from)?;
        ...
        let _ = self.cache.zadd(&index_key, now as f64, self.node_id.as_bytes()).await.map_err(errors::Error::from)?;
        
        Ok(())
    }

    /// Retrieves a list of all currently active nodes in the cluster.
    pub async fn get_active_nodes(&self) -> Result<Vec<NodeInfo>, errors::Error> {
        let now = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs();
        ...
        let active_ids_bytes = self.cache.zrangebyscore(&index_key, cutoff as f64, f64::INFINITY).await.map_err(errors::Error::from)?;
        ...
        let results = self.cache.mget(&key_refs).await.map_err(errors::Error::from)?;
    ...
    pub async fn deregister(&self) -> Result<(), errors::Error> {
