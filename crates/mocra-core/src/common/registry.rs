...
impl NodeRegistry {
    ...
    pub async fn heartbeat(
        ...
        info.send_with_ttl(&self.node_id, &self.cache, self.ttl)
            .await
        ...
        let _ = self
            .cache
            .zadd(&index_key, now as f64, self.node_id.as_bytes())
            .await
            .map_err(crate::errors::Error::from)?;
    ...
    pub async fn get_active_nodes(&self) -> Result<Vec<NodeInfo>, crate::errors::Error> {
        ...
        let _ = self
            .cache
            .zremrangebyscore(&index_key, f64::NEG_INFINITY, cutoff as f64)
            .await;

        // 2. Get active Node IDs
        let active_ids_bytes = self
            .cache
            .zrangebyscore(&index_key, cutoff as f64, f64::INFINITY)
            .await
            .map_err(crate::errors::Error::from)?;
