...
async fn test_local_isolation() {
    ...
    let service1 = crate::sync::SyncService::from_config(None, "ns1".to_string(), &config);
    let service2 = crate::sync::SyncService::from_config(None, "ns2".to_string(), &config);
...
async fn test_distributed_isolation() {
    ...
    let redis_backend = Arc::new(crate::sync::RedisBackend::new(REDIS_URL));
    ...
    let config = crate::common::model::config::SyncConfig::default();
    let service1 = crate::sync::SyncService::from_config(Some(redis_backend.clone()), ns1.clone(), &config);
    let service2 = crate::sync::SyncService::from_config(Some(redis_backend.clone()), ns2.clone(), &config);
