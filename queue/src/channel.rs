...
pub struct Channel {
    ...
    // 远程/分布式节点通信队列
    pub remote_response_sender: Sender<Response>,
    pub remote_response_receiver: Arc<Mutex<Receiver<Response>>>,

    pub remote_parser_task_sender: Sender<ParserTaskModel>,
    pub remote_parser_task_receiver: Arc<Mutex<Receiver<ParserTaskModel>>>,

    pub remote_error_sender: Sender<ErrorTaskModel>,
    pub remote_error_receiver: Arc<Mutex<Receiver<ErrorTaskModel>>>,
}
...
impl Channel {
    pub fn new(capacity: usize) -> Self {
        ...
        // Remote channels
        let (remote_response_sender, remote_response_receiver) = channel(capacity);
        let (remote_parser_task_sender, remote_parser_task_receiver) = channel(capacity);
        let (remote_error_sender, remote_error_receiver) = channel(capacity);
