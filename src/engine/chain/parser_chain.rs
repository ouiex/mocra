    ...
    async fn process(
        ...
        for task in data.parser_task.drain(..) {
             ...
             if is_same_module && is_same_context {
                 ...
                 let dag_flags = module_clone.dag_runtime_flags();

                 if dag_flags.dry_run {
                     match module_clone.run_dag_preflight_dry_run().await {
                         Ok(_) => {
                             counter!(
                                 "module_dag_preflight_total",
                                 "result" => "ok".to_string(),
                                 "module" => module_clone.module.name()
                             )
                             .increment(1);
                         }
                         Err(err) => {
                             warn!(
                                 "[ResponseParserProcessor] module dag dry-run preflight failed: module={} err={}",
                                 module_clone.module.name(),
                                 err
                             );
                             counter!(
                                 "module_dag_preflight_total",
                ...
                let cutover_scope = TaskManager::module_cutover_scope(&module_clone);
                 ...
                 if dag_flags.shadow_execute {
                         ...
                         .await
                     {
                         Ok(compare) => {
                             ...
                             .increment(1);
                             debug!(
                                 "[ResponseParserProcessor] module dag shadow execute succeeded: module={} outputs={} node_results={} expected_steps={} shadow_steps={} compare={}",
                                 module_clone.module.name(),
                         ...
                         Err(err) => {
                             ...
                             .increment(1);
                             warn!(
                                 "[ResponseParserProcessor] module dag shadow execute failed: module={} err={}",
                                 module_clone.module.name(),
                                 err
                             );
                             self.task_manager
                     ...
                     if dag_flags.primary_preview {
                         if dag_flags.cutover_requested && !dag_flags.cutover_supported {
                             ...
                             .increment(1);
                             warn!(
                                 "[ResponseParserProcessor] module dag cutover requested but module not in cutover whitelist: module={} (fallback to preview/legacy)",
                                 module_clone.module.name()
                             );
                         }

                         let cutover_threshold = module_clone
                             .config
                             .get_config_value("module_dag_cutover_fail_threshold")
                             .and_then(|v| v.as_u64())
                             .unwrap_or(3) as usize;
                         let cutover_recovery_window_secs = module_clone
                             .config
                         ...
                         if dag_flags.primary_cutover && !cutover_allowed {
                             ...
                             .increment(1);
                             warn!(
                                 "[ResponseParserProcessor] module dag cutover blocked: module={} threshold={} recovery_window_secs={} shadow_gate_allowed={} warmup_gate_allowed={} min_shadow_matches={} min_observation_window_secs={} (fallback to legacy generate)",
                                 module_clone.module.name(),
                                 cutover_threshold,
                                 cutover_recovery_window_secs,
                                 cutover_shadow_gate_allowed,
                         ...
                         match self
                             .task_manager
                             .execute_module_dag_linear_compat(module_clone.module.name().as_str())
                             .await
                         {
                             Ok(report) => {
                                 counter!(
                                 ...
                                 if dag_flags.primary_cutover {
                                     if cutover_allowed {
                                         ...
                                         .increment(1);
                                         self.task_manager.record_module_dag_cutover_success(
                                             cutover_scope.as_str(),
                                         );
                                     }
                                 }
                                 debug!(
                                     "[ResponseParserProcessor] module dag primary preview succeeded: module={} outputs={} node_results={} (legacy generate kept for compatibility)",
                                     module_clone.module.name(),
                                     report.outputs.len(),
                             ...
                             Err(err) => {
                                 ...
                                 if dag_flags.primary_cutover {
                                     if cutover_allowed {
                                         ...
                                         .increment(1);
                                         self.task_manager.record_module_dag_cutover_failure(
                                             cutover_scope.as_str(),
                                         );
                                     }
                                 }
                                 warn!(
                                     "[ResponseParserProcessor] module dag primary preview failed, fallback to legacy generate: module={} err={}",
                                     module_clone.module.name(),
                                     err
                                 );
                             }
                 ...
                 } else if dag_flags.primary_preview {
                     if dag_flags.cutover_requested && !dag_flags.cutover_supported {
                         counter!(
                             "module_dag_cutover_total",
                             "result" => "blocked_not_supported".to_string(),
                     ...
                     if dag_flags.primary_cutover && !cutover_allowed {
                         counter!(
                             "module_dag_cutover_total",
                             "result" => "auto_downgraded".to_string(),
                             "module" => module_clone.module.name()
                         )
                         .increment(1);
                         warn!(
                             "[ResponseParserProcessor] module dag cutover auto-downgraded by failure threshold: module={} threshold={} recovery_window_secs={} (fallback to legacy generate)",
                             module_clone.module.name(),
                             cutover_threshold,
                             cutover_recovery_window_secs
                         );
